...
from samples.vsphere.vcenter.helper.guest_helper import wait_for_guest_info_ready
...
class VcenterRangeClient:
    def __init__(self, conf_file, yaml_file):
        ...
        self.vsphere_client = create_vsphere_client(server=config['server'],
                                                username=config['user'],
                                                password=config['pass'], session=session)
        self.servicemanager = ServiceManagerFactory.get_service_manager(config['server'],
                                                                    config['user'],
                                                                    config['pass'],
                                                                    skip_verification=True)

    def list_vms(self):
            list = self.vsphere_client.vcenter.VM.list()
            pprint(list)
            return list
    ...
    def get_network_id(self,network_name):
        ...
        """
        datacenter = datacenter_helper.get_datacenter(self.vsphere_client, self.template['prereqs']['datacenter_name'])
        if not datacenter:
            print("Datacenter '{}' not found".format(self.template['prereqs']['datacenter_name']))
            return None

        filter = Network.FilterSpec(datacenters=set([datacenter]),
                                    names=set([network_name]))
        network_summaries = self.vsphere_client.vcenter.Network.list(filter=filter)
        ...
        if len(network_summaries) > 0:
            ...
            print("Portgroup Network not found in Datacenter '{}'".
                format(self.template['prereqs']['datacenter_name']))
            return None


    def delete_vm(self, vm_name):
            """
                Delete a VM using a predefined vsphere_client.
                @param vsphere_client: vsphere_client defined by vmware.vapi.vsphere.client.create_vsphere_client()
                @param vm_name: name of VM to be deleted
            """
            vm = get_vm(self.vsphere_client, vm_name)
            if not vm:
    ...
    def power_suspend(self, vm_name):
        vm = get_vm(self.vsphere_client, vm_name)
    ...
    def get_guest_info(self, vm_name, force_power_on=False):
            vm = get_vm(self.vsphere_client, vm_name)
            if not vm:
                raise Exception('Sample requires an existing vm with name ({}).'
                                'Please create the vm first.'.format(vm_name))
            ...
            if status != Power.Info(state=Power.State.POWERED_ON) and force_power_on:
                ...
                raise Exception('The VM you specified is turned off. '+
                                'To turn on, try again by specifying get_guest_info(vsphere_client, vm_name, force_power_on=True')

            # wait for guest info to be ready
            wait_for_guest_info_ready(self.vsphere_client, vm, 600)

            # get the Identity
            identity = self.vsphere_client.vcenter.vm.guest.Identity.get(vm)
            print('vm.guest.Identity.get({})'.format(vm))
            print('Identity: {}'.format(pp(identity)))

            # get the local filesystem info
            local_filesysteem = self.vsphere_client.vcenter.vm.guest.LocalFilesystem.get(vm)
            print('vm.guest.LocalFilesystem.get({})'.format(vm))
            print('LocalFilesystem: {}'.format(pp(local_filesysteem)))

            # get the Network
            network = self.vsphere_client.vcenter.vm.guest.Networking.get(vm)
            print('vm.guest.Networking.get({})'.format(vm))
            print('Networking: {}'.format(pp(network)))

            nic_summaries = self.vsphere_client.vcenter.vm.hardware.Ethernet.list(vm)
            for nic_summary in nic_summaries:
                nic = nic_summary.nic
                nic_info = self.vsphere_client.vcenter.vm.hardware.Ethernet.get(vm=vm, nic=nic)
                print('vm.hardware.Ethernet.get({}, {}) -> {}'.
                    format(vm, nic, nic_info))

            # get the Customization
            cust = self.vsphere_client.vcenter.vm.guest.Customization.get(vm)
            print('\nvm.guest.Customization.get({})'.format(vm))
            print('Customization: {}'.format(pp(cust)))
            
            # get the Operations
            cust = self.vsphere_client.vcenter.vm.guest.Operations.get(vm)
            print('vm.guest.Operations.get({})'.format(vm))
            print('Operations: {}'.format(pp(cust)))

            ip = self.get_ip(vm_name)
            mac = self.get_macs(vm_name)
    ...
    def get_macs(self,vm_name):
            ...
            nic_list = self.vsphere_client.vcenter.vm.hardware.Ethernet.list(vm)
            mac_list = []
            for i in range(len(nic_list)):
                mac_list.append(self.vsphere_client.vcenter.vm.hardware.Ethernet.get(vm,nic_list[i].nic).mac_address)

            return mac_list
            

    def create_vm_from_iso(self,turn_on=False):
            placement_spec = vm_placement_helper.get_placement_spec_for_resource_pool(
                        self.vsphere_client,
                        self.template['prereqs']['datacenter_name'],
                        self.template['prereqs']['folder_name'],
                        self.template['prereqs']['datastore_name'])

            iso_datastore_path = "[" + self.template['prereqs']['datastore_name'] + "] " + self.template['vm']['iso_path']
            
            GiB = 1024 * 1024 * 1024
            GiBMemory = 1024

            disk_specs = []
            for i in range(self.template['vm']['disks']):
                    current_disk = "disk" + str(i+1)
                    disk_specs.append(Disk.CreateSpec(new_vmdk=Disk.VmdkCreateSpec(name=self.template['vm'][current_disk]['name'],
                                                    capacity=self.template['vm'][current_disk]['capacity'] * GiB)))
                


            nic_specs = []
            for i in range(self.template['vm']['nics']):
                ...
                nic_specs.append(Ethernet.CreateSpec(
                        ...
                        backing=Ethernet.BackingSpec(
                            ...
                            network=self.get_network_id(self.template['vm'][current_nic]['network']))))



            vm_create_spec = VM.CreateSpec(
                guest_os=self.template['vm']['guest_os'],
                name=self.template['vm']['vm_name'],
                placement=placement_spec,
                hardware_version=self.template['vm']['hardware_version'],
                cpu=Cpu.UpdateSpec(count=self.template['vm']['cpu_count'],
                                cores_per_socket=1,
                                hot_add_enabled=False,
                                hot_remove_enabled=False),
                memory=Memory.UpdateSpec(size_mib=self.template['vm']['memory'] * GiBMemory,
                                        hot_add_enabled=False),
                disks=disk_specs,
                nics=nic_specs,
                cdroms=[
            ...
            print("create_exhaustive_vm: Created VM '{}' ({})".format(self.template['vm']['vm_name'],vm))
            
            vm_obj = get_obj_by_moId(self.servicemanager.content,[vim.VirtualMachine], vm)
            print("Deployed VM '{0}' with ID: {1}".format(self.template['vm']['vm_name'],vm))
            ...
            if turn_on:
                self.power_on(self.template['vm']['vm_name'])
            return vm
    ...
    #confirmed
    def import_ova_to_ovf(self):
            cls_client = ClsApiClient(self.servicemanager)
            ...
            datastore_name = self.template['prereqs']['datastore_name']
            storage_backings = helper.create_storage_backings(service_manager=self.servicemanager,
                                                                datastore_name=datastore_name)


            if self.template['vm']['existing_library_id'] == "None":
                # Create a local content library backed by the VC datastore using vAPIs
                local_lib_id = helper.create_local_library(storage_backings, self.template['vm']['lib_name'])
                print('Content Library {0} created. ID: {1}'.format(self.template['vm']['lib_name'],local_lib_id))
            else:
                local_lib_id = self.template['vm']['existing_library_id']
            ...
            lib_item_id = helper.create_library_item(library_id=local_lib_id,
                                                            item_name=self.template['vm']['template_name'],
                                                            item_description='Sample template from ova file',
                                                            item_type='ovf')
            print('Library item created. ID: {0}'.format(lib_item_id))

            ova_file_map = helper.get_ova_file_map(self.template['vm']['ova_current_full_path'],
                                                        local_filename=self.template['vm']['current_ova_name'])
    ...
    def create_vm_from_ovf(self):
            ...
            lib_item_id = helper.get_item_id_by_name(self.template['vm']['template_name'])
            assert lib_item_id
            ovf_summary = cls_client.ovf_lib_item_service.filter(ovf_library_item_id=lib_item_id,
            ...
            result = cls_client.ovf_lib_item_service.deploy(lib_item_id,
                                                            deployment_target,
                                                            deployment_spec,
                                                            client_token=generate_random_uuid())
            ...
            if result.succeeded:
                ...
                vm = get_obj_by_moId(self.servicemanager.content,[vim.VirtualMachine], vm_id)
                print("Deployed VM '{0}' with ID: {1}".format(self.template['vm']['vm_name'],vm_id))
                ...
                poweron_vm(self.servicemanager.content, vm_obj)
    ...
    def update_networking(self):
            vm = get_vm(self.vsphere_client, self.template['vm']['vm_name'])
            nic_list = self.vsphere_client.vcenter.vm.hardware.Ethernet.list(vm)
            print(nic_list)
            for i in range(len(nic_list)):
                print("Cleaning up {}".format(nic_list[i].nic))
                self.vsphere_client.vcenter.vm.hardware.Ethernet.delete(vm,nic_list[i].nic)
            ...
            for i in range(self.template['vm']['nics']):
                ...
                nic = Ethernet.CreateSpec(
                        ...
                        backing=Ethernet.BackingSpec(
                            type=Ethernet.BackingType.STANDARD_PORTGROUP,
                            network=self.get_network_id(self.template['vm'][current_nic]['network'])))
                self.vsphere_client.vcenter.vm.hardware.Ethernet.create(vm, nic)

            
            vm_obj = get_obj_by_moId(self.servicemanager.content,[vim.VirtualMachine], vm)
            print("Updated VM '{0}' with ID: {1}".format(self.template['vm']['vm_name'],vm))
    ...
    def clone_to_template(self):
            ...
            # Get the identifiers
            vm_id = get_vm(self.vsphere_client, self.template['vm']['vm_name'])
            placement_spec = vm_placement_helper.get_placement_spec_for_resource_pool(
                        self.vsphere_client,
                        self.template['prereqs']['datacenter_name'],
                        self.template['prereqs']['folder_name'],
            ...
            # Create a library
            if self.template['vm']['existing_library_id'] == "None":
                storage_backings = helper.create_storage_backings(self.servicemanager, self.template['prereqs']['datastore_name'])
                library_id = helper.create_local_library(storage_backings,self.template['vm']['lib_name'])
            else:
                library_id = self.template['vm']['existing_library_id']
            ...
            create_spec.name = self.template['vm']['template_name']
    ...
    def create_vm_from_template(self,turn_on=False):
            ...
            item_id = helper.get_item_id_by_name(self.template['vm']['template_name'])
            ...
            vm_id = cls_client.vmtx_service.deploy(item_id, deploy_spec)
            vm = get_obj_by_moId(self.servicemanager.content,[vim.VirtualMachine], vm_id)
            print("Deployed VM '{0}' with ID: {1}".format(vm.name,vm_id))

            self.update_networking()
            self.disconnect_cdroms()
    ...
    def disconnect_cdroms(self):
            vm = get_vm(self.vsphere_client, self.template['vm']['vm_name'])
            cdrom_summaries = self.vsphere_client.vcenter.vm.hardware.Cdrom.list(vm=vm)
            
            for i in range(len(cdrom_summaries)):
                print(self.vsphere_client.vcenter.vm.hardware.Cdrom.get(vm, cdrom_summaries[i].cdrom))
                self.vsphere_client.vcenter.vm.hardware.Cdrom.delete(vm, cdrom_summaries[i].cdrom)

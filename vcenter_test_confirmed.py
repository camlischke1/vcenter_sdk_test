...
def get_network_id(vsphere_client,
                        network_name,
                        datacenter_name):
    """
    ...
    """
    datacenter = datacenter_helper.get_datacenter(vsphere_client, datacenter_name)
    if not datacenter:
...
# confirmed
def delete_vm(vsphere_client, vm_name):
        """
...
# confirmed
def power_on(vsphere_client, vm_name):
    vm = get_vm(vsphere_client, vm_name)
...
# confirmed
def power_off(vsphere_client, vm_name):
    vm = get_vm(vsphere_client, vm_name)
...
def get_state(vsphere_client,vm_name):
    vm = get_vm(vsphere_client, vm_name)
...
def power_suspend(vsphere_client, vm_name):
    vm = get_vm(vsphere_client, vm_name)
    ...
    if status == Power.Info(state=Power.State.POWERED_ON):
        ...
        vsphere_client.vcenter.vm.Power.suspend(vm)
        print('vm.Power.suspend({})'.format(vm))



#confirmed
def get_guest_info(vsphere_client, vm_name, force_power_on=False):
        vm = get_vm(vsphere_client, vm_name)
        if not vm:
            raise Exception('Sample requires an existing vm with name ({}).'
                            'Please create the vm first.'.format(vm_name))
        ...
        wait_for_guest_info_ready(vsphere_client, vm, 600)

        # get the Identity
        identity = vsphere_client.vcenter.vm.guest.Identity.get(vm)
        print('vm.guest.Identity.get({})'.format(vm))
        print('Identity: {}'.format(pp(identity)))

        # get the local filesystem info
        local_filesysteem = vsphere_client.vcenter.vm.guest.LocalFilesystem.get(vm)
        print('vm.guest.LocalFilesystem.get({})'.format(vm))
        print('LocalFilesystem: {}'.format(pp(local_filesysteem)))

        # get the Network
        network = vsphere_client.vcenter.vm.guest.Networking.get(vm)
        print('vm.guest.Networking.get({})'.format(vm))
        print('Networking: {}'.format(pp(network)))

        nic_summaries = vsphere_client.vcenter.vm.hardware.Ethernet.list(vm)
        for nic_summary in nic_summaries:
            nic = nic_summary.nic
            nic_info = vsphere_client.vcenter.vm.hardware.Ethernet.get(vm=vm, nic=nic)
            print('vm.hardware.Ethernet.get({}, {}) -> {}'.
                format(vm, nic, nic_info))

        # get the Customization
        cust = vsphere_client.vcenter.vm.guest.Customization.get(vm)
        print('\nvm.guest.Customization.get({})'.format(vm))
        print('Customization: {}'.format(pp(cust)))
        
        # get the Operations
        cust = vsphere_client.vcenter.vm.guest.Operations.get(vm)
        print('vm.guest.Operations.get({})'.format(vm))
        print('Operations: {}'.format(pp(cust)))

        ip = get_ip(vsphere_client,vm_name)
        mac = get_macs(vsphere_client,vm_name)
        print("Quick Access: \n Name: {}\n IP: {}\n MACS: {}".format(vm_name,ip,mac))


#confirmed
def get_ip(vsphere_client,vm_name):
        vm = get_vm(vsphere_client, vm_name)
        if not vm:
            raise Exception('Sample requires an existing vm with name ({}).'
                            'Please create the vm first.'.format(vm_name))
...
#confirmed
def get_macs(vsphere_client,vm_name):
        vm = get_vm(vsphere_client, vm_name)
        if not vm:
            raise Exception('Sample requires an existing vm with name ({}).'
                            'Please create the vm first.'.format(vm_name))
        ...
        nic_list = vsphere_client.vcenter.vm.hardware.Ethernet.list(vm)
        mac_list = []
        for i in range(len(nic_list)):
            mac_list.append(vsphere_client.vcenter.vm.hardware.Ethernet.get(vm,nic_list[i].nic).mac_address)
        ...
        for i in range(template['vm']['nics']):
            ...
            nic_specs.append(Ethernet.CreateSpec(
                    ...
                    backing=Ethernet.BackingSpec(
                        ...
                        network=get_network_id(vsphere_client,template['vm'][current_nic]['network'],template['prereqs']['datacenter_name']))))
...
def update_networking(yaml_file,conf_file):
        ...
        for i in range(len(nic_list)):
            ...
            vsphere_client.vcenter.vm.hardware.Ethernet.delete(vm,nic_list[i].nic)
        ...
        for i in range(template['vm']['nics']):
            ...
            nic = Ethernet.CreateSpec(
                    ...
                    backing=Ethernet.BackingSpec(
                        type=Ethernet.BackingType.STANDARD_PORTGROUP,
                        network=get_network_id(vsphere_client,template['vm'][current_nic]['network'],template['prereqs']['datacenter_name'])))
            vsphere_client.vcenter.vm.hardware.Ethernet.create(vm, nic)
